    ...
    switch($Operation)
    {
      ...
      if(!empty($NewSiteSectionName) && !empty($NewSiteSectionDirectory) && !empty($NewSiteSectionTitle) && !empty($NewSiteSectionKeywords) && !empty($NewSiteSectionDescription))
      {
        ...
        if(mysqli_affected_rows($MainConnection))
        {
          ...
          //make directories
          umask(0);
          if(mkdir($ApplicationPath.'/'.$NewSiteSectionDirectory,0777) && mkdir($ApplicationPath.'/'.$NewSiteSectionDirectory.'/query',0777) && mkdir($ApplicationPath.'/'.$NewSiteSectionDirectory.'/display',0777)){$Message.= '<span class="AlertText"><br />New directory \''.$NewSiteSectionDirectory.'\' created.<br /></span>';}else{$Message.= '<span class="AlertText"><br />New directory \''.$NewSiteSectionDirectory.'\' could not be created.<br /></span>';}
          if(mkdir($ApplicationPath.'/'.$NewSiteSectionDirectory.'/cache')){$Message.= '<span class="AlertText"><br />Cache directory created</span>';}else{$Message.= '<span class="AlertText"><br />Cache directory could not be created</span>';}

          //copy files
          if(copy($ApplicationPath.'/template/defaults/cached.php',$ApplicationPath.'/'.$NewSiteSectionDirectory.'/cache/cached.php')){$Message.= '<span class="AlertText"><br />Cache file created</span>';}else{$Message.= '<span class="AlertText"><br />Cache file could not be created</span>';}
          if(copy($ApplicationPath.'/template/defaults/index',$ApplicationPath.'/'.$NewSiteSectionDirectory.'/index')){$Message.= '<span class="AlertText"><br />Directory index file created</span>';}else{$Message.= '<span class="AlertText"><br />Directory index file could not be created</span>';}
          if(copy($ApplicationPath.'/template/defaults/main.php',$ApplicationPath.'/'.$NewSiteSectionDirectory.'/'.$NewSiteSectionDirectory.'.php')){$Message.= '<span class="AlertText"><br />Directory default page created</span><br />';}else{$Message.= '<span class="AlertText"><br />Directory default page could not be created</span><br />';}
